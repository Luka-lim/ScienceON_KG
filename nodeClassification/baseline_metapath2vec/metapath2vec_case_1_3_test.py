...
model = torch.load('./model/metapath2vec_case_1_3_idx_200.pt', map_location=device)

print('load train/test data')
dfReportGT = pd.read_csv('../SATDataset_ver2.1_a/Report_Label.csv')
X_data = model('report', batch=torch.from_numpy(dfReportGT['IDREPORT'].values))

# report y index
classes = []
for idx, item in dfReportGT.iterrows():
    cls = [item['ClassID']]
    classes.extend(cls)
...
Y_data = torch.zeros((dfReportGT.shape[0], num_classes), dtype=torch.float)
for i, item in dfReportGT.iterrows():
...
for roundIDX in range(1, rounds):
    ...
    micro_1_3_1.to_csv('./model/metapath2vec_micro_1_3_1.csv', encoding='utf-8-sig', index=False)
    macro_1_3_1.to_csv('./model/metapath2vec_macro_1_3_1.csv', encoding='utf-8-sig', index=False)
...
for roundIDX in range(1, rounds):
    ...
    micro_1_3_2.to_csv('./model/metapath2vec_micro_1_3_2.csv', encoding='utf-8-sig', index=False)
    macro_1_3_2.to_csv('./model/metapath2vec_macro_1_3_2.csv', encoding='utf-8-sig', index=False)
